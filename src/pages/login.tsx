...
export default function Login() {
  ...
  // Se o usuário estiver autenticado, redirecionar para o dashboard ou página especificada
  useEffect(() => {
  ...
  // Verificar se há um motivo para exibir mensagem especial
  useEffect(() => {
    if (reason === "session_expired") {
      setError("Sua sessão expirou. Por favor, faça login novamente.");
      toast.error("Sessão expirada", { id: "session-expired" });
    }
  }, [reason]);

  // Limpar sessão anterior quando o componente montar
  useEffect(() => {
    // Se tivermos um motivo de redirecionamento, fazer logout para limpar o estado
    if (reason === "session_expired" && isAuthenticated) {
      console.log("Login - Limpando sessão anterior devido a redirecionamento");
      logout();
    }
  }, [reason, isAuthenticated, logout]);
          ...
          <div className="h-full flex flex-col justify-between relative">
            ...
            <div>
              ...
              <p className="text-lg text-white/90">
                Tudo que você precisa para gerenciar seu negócio de qualquer
                lugar
              </p>
            </div>
          </div>
